...
# 从 LangChain 导入文档和消息结构（已修正为新版 langchain_core 路径）
...
def render_main_chat_ui():
    ...
    with tab1:
        ...
        for message in st.session_state.messages:
            # 优化：为了保证前端 UI 图标正确渲染，将 LangChain 消息类显式转换为 Streamlit 角色名
            avatar_role = "user" if isinstance(message, HumanMessage) else "assistant"
            ...
            with st.chat_message(avatar_role):
                ...
                if isinstance(message, AIMessage):
                    ...
                    try:
                        ...
                        # 如果解析失败（如闲聊、无合适简历时的纯文本兜底），则当作纯文本处理
                    ...
                    if isinstance(content_data, dict):
                        ...
                        if isinstance(candidates, list):
                            render_candidate_cards(candidates, title=None)  # title=None 避免重复打印标题

                        # 如果结构是字典但没有核心字段，安全回退到打印原始文本
                        elif not response_text and not candidates:
                            st.markdown(message.content)
                    else:
                        # 兼容内容是非字典形态的合法 JSON 对象
                        st.markdown(message.content)
                else:
                    # 如果是用户的消息，直接显示
                    st.markdown(message.content)
            ...
            with st.chat_message("assistant"):
                with st.spinner("AI 正在分析您的需求并匹配简历..."):
                    try:
                        ...
                        #    就用它更新session_state中的last_candidates，为下一次追问/深挖对话做准备
    ...
    with tab2:
        ...
        if uploaded_file:
            ...
            try:
                ...
                # 如果处理过程中发生任何错误，向用户显示错误信息并进行文件物理清理
                ...
                if os.path.exists(file_path):
                    os.remove(file_path)  # 发生故障则清理脏文件
...
# --- 5. 主程序入口 ---
# 这是一个简单的路由逻辑，根据 URL 中的查询参数来决定显示主聊天界面还是单独的简历 PDF/文本浏览器
if "view_resume" in st.query_params:
    # 如果 URL 是 .../?view_resume=xxx，则直接渲染单个简历详细视图
